...
export function WorkspaceBoard({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <section className="glass" style={{ padding: 32, borderRadius: 24, position: "relative", overflow: "hidden" }}>
        {paymentStatus === "unpaid" ? (
          <div style={{ position: "absolute", top: 0, left: 0, width: 4, height: "100%", background: "var(--accent-rose)", boxShadow: "var(--glow-rose)" }} />
        ) : (
          <div style={{ position: "absolute", top: 0, left: 0, width: 4, height: "100%", background: "var(--accent-emerald)", boxShadow: "var(--glow-emerald)" }} />
        )}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 24 }}>
          <div>
            <h3 style={{ margin: "0 0 8px 0", color: "var(--text-bright)", fontSize: 24, fontWeight: 800 }}>
              Milestone Treasury
            </h3>
          ...
          <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
            {paymentStatus === "unpaid" && partnerPaymentLink && (
              <a 
                ...
                className="pill-btn"
                style={{ background: "rgba(255,255,255,0.05)", border: "1px solid var(--stroke)", color: "var(--text-bright)", fontWeight: 700 }}
              >
                Initiate Transfer ↗
              </a>
            )}
            {paymentStatus === "unpaid" && !partnerPaymentLink && (
              <span className="sub" style={{ fontSize: 13, padding: "8px 16px", background: "rgba(255,255,255,0.02)", borderRadius: 32, border: "1px solid var(--stroke)" }}>
                Awaiting Routing Link
              </span>
            )}
            {paymentStatus === "paid" ? (
              <button className="pill-btn accept" disabled style={{ background: "rgba(0, 255, 179, 0.1)", color: "var(--accent-emerald)", border: "1px solid var(--accent-emerald)", opacity: 1, boxShadow: "0 0 16px rgba(0, 255, 179, 0.2)", cursor: "default" }}>
                Funds Secured ✔
              </button>
            ) : (
              <button className="pill-btn" onClick={handlePayment} disabled={pending} style={{ background: "var(--text-bright)", color: "#000", fontWeight: 800 }}>
                {pending ? "Syncing..." : "Mark as Transferred"}
              </button>
            )}
          </div>
        </div>
        {error ? <p className="error" style={{ marginTop: 16, padding: "12px", background: "rgba(255,61,120,0.1)", borderRadius: 8, border: "1px solid var(--accent-rose)" }}>{error}</p> : null}
      </section>

      {/* Role-based Dynamic Hub Blocks */}
      {categories.includes("Software & IT") && (
        <section className="glass" style={{ padding: 32, borderRadius: 24 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16, marginBottom: embeds.some(e => e.embed_type === 'github') ? 24 : 0 }}>
            <div>
              <h3 style={{ margin: "0 0 8px 0", color: "var(--text-bright)", fontSize: 20, fontWeight: 800 }}>Developer Terminal</h3>
          ...
          {embeds.filter(e => e.embed_type === 'github').length > 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {embeds.filter(e => e.embed_type === 'github').map(e => (
                <a key={e.id} href={e.url} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", padding: "16px", background: "rgba(0,0,0,0.3)", border: "1px solid var(--stroke)", borderRadius: 16, color: "var(--text-bright)", textDecoration: "none", transition: "all 0.2s ease" }} className="hover:border-cyan-500 hover:bg-cyan-900/10">
                  ...
                  <span style={{ color: "var(--accent-cyan)", fontSize: 13, fontWeight: 700 }}>EXECUTE ↗</span>
                </a>
              ))}
            </div>
          )}
      ...
      {categories.includes("Creative & Design") && (
        <section className="glass" style={{ padding: 32, borderRadius: 24 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16, marginBottom: embeds.some(e => e.embed_type === 'figma') ? 24 : 0 }}>
            <div>
              <h3 style={{ margin: "0 0 8px 0", color: "var(--text-bright)", fontSize: 20, fontWeight: 800 }}>Design Studio</h3>
              <p className="sub" style={{ margin: 0, fontSize: 14 }}>
          ...
          {embeds.filter(e => e.embed_type === 'figma').length > 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              {embeds.filter(e => e.embed_type === 'figma').map(e => {
                ...
                return (
                  <div key={e.id} style={{ borderRadius: 16, overflow: "hidden", border: "1px solid var(--accent-purple)", boxShadow: "0 0 32px rgba(180, 77, 255, 0.15)" }}>
                    <iframe 
                      src={embedUrl}
      ...
      {(categories.includes("Business & Sales") || categories.includes("Marketing & Content")) && (
        <section className="glass" style={{ padding: 32, borderRadius: 24 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16, marginBottom: embeds.some(e => e.embed_type === 'notion') ? 24 : 0 }}>
            <div>
              <h3 style={{ margin: "0 0 8px 0", color: "var(--text-bright)", fontSize: 20, fontWeight: 800 }}>Operations Hub</h3>
              <p className="sub" style={{ margin: 0, fontSize: 14 }}>
          ...
          {embeds.filter(e => e.embed_type === 'notion').length > 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {embeds.filter(e => e.embed_type === 'notion').map(e => (
                <a key={e.id} href={e.url} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", padding: "16px", background: "rgba(0,0,0,0.3)", border: "1px solid var(--stroke)", borderRadius: 16, color: "var(--text-bright)", textDecoration: "none", transition: "all 0.2s ease" }} className="hover:border-purple-500 hover:bg-purple-900/10">
                  <span style={{ marginRight: 16, fontSize: 20 }}>📝</span>
                  <span style={{ flexGrow: 1, fontFamily: "var(--font-mono)", fontSize: 14 }}>{e.url.replace(/^https?:\/\/(www\.)?/, "")}</span>
                  <span style={{ color: "var(--accent-purple)", fontSize: 13, fontWeight: 700 }}>OPEN ↗</span>
                </a>
              ))}
            </div>
          )}
      ...
      <section 
        ...
        onDragOver={(e) => { e.preventDefault(); e.stopPropagation(); e.currentTarget.style.borderColor = "var(--accent-emerald)"; e.currentTarget.style.background = "rgba(0,255,179,0.05)"; }}
        onDragLeave={(e) => { e.currentTarget.style.borderColor = "var(--stroke-subtle)"; e.currentTarget.style.background = "rgba(18, 18, 31, 0.4)"; }}
        onDrop={(e) => {
          e.preventDefault();
          e.stopPropagation();
          e.currentTarget.style.borderColor = "var(--stroke-subtle)"; e.currentTarget.style.background = "rgba(18, 18, 31, 0.4)";
      ...
      >
        ...
        <div>
          <h3 style={{ margin: "0 0 8px 0", color: "var(--text-bright)", fontSize: 20, fontWeight: 800 }}>Asset Dropzone</h3>
          <p className="sub" style={{ margin: 0, fontSize: 14 }}>
            Images, PDFs, docs. Drag & drop to securely transfer.
          </p>
        </div>
        <label className="pill-btn" style={{ cursor: pending ? "wait" : "pointer", background: "var(--text-bright)", color: "#000", fontWeight: 800 }}>
          ...
          {files.map((f) => (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, border: "1px solid var(--stroke-subtle)" }}>
                  📄
                </div>
                <div>
                  <div style={{ fontWeight: 700, color: "var(--text-bright)", fontSize: 15, marginBottom: 4 }}>{f.file_name}</div>
