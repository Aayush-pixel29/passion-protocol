  ...
  return (
    <div style={{ maxWidth: 640, margin: "0 auto", position: "relative", overflow: "hidden", padding: "0 16px" }}>
      ...
      <form action={action} style={{ position: "relative", zIndex: 10 }}>
        ...
        {/* Step 1: Identity */}
        {step === 1 && (
          <div className="animate-slide-up glass onboarding-card" style={{ padding: 40, borderRadius: 24 }}>
                ...
                {AVATAR_OPTIONS.map(name => (
                  <div key={name} 
                       ...
                       style={{ 
                         ...
                         background: selectedAvatar === name ? 'rgba(0, 255, 179, 0.05)' : 'rgba(255,255,255,0.02)',
                         cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 8,
                         boxShadow: selectedAvatar === name ? 'var(--glow-emerald)' : 'none'
                       }}>
            ...
            <label>
              ...
              <input name="codename" className="input glass" value={codename} onChange={e => setCodename(e.target.value)} required pattern="[A-Za-z0-9_ ]{2,32}" placeholder="e.g. GhostStack, NullBase, Vektor..." style={{ background: "rgba(255,255,255,0.9)", color: "#000" }} />
            ...
            <label style={{ marginTop: 24 }}>
              ...
              <select className="input glass" value={category} onChange={e => setCategory(e.target.value as IndustryCategory)} style={{ appearance: "none" }}>
            ...
            <button type="button" className="pill-btn" style={{ marginTop: 32, width: "100%", background: "var(--accent-emerald)", color: "#000", fontWeight: 700 }} onClick={() => setStep(2)}>
              Continue &rarr;
            </button>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="animate-slide-up glass onboarding-card" style={{ padding: 40, borderRadius: 24 }}>
            ...
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 32 }}>
              <div style={{ padding: 24, borderRadius: 24, border: "1px solid var(--accent-purple)", background: "rgba(180, 77, 255, 0.05)", display: "flex", flexDirection: "column", alignItems: "center", gap: 12, boxShadow: "var(--glow-purple)" }}>
        ...
        {step === 3 && (
          <div className="animate-slide-up glass onboarding-card" style={{ padding: 40, borderRadius: 24 }}>
            ...
            <div style={{ display: "flex", gap: 16, marginTop: 16 }}>
              ...
              <button type="button" className="pill-btn" style={{ flex: 1, background: "var(--accent-rose)", color: "#fff", fontWeight: 700 }} onClick={() => setStep(4)}>Finalize &rarr;</button>
            </div>
          </div>
        )}
        ...
        {/* Step 4: Ready */}
        {step === 4 && (
          <div className="animate-slide-up glass onboarding-card" style={{ padding: 40, borderRadius: 24, textAlign: "center" }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 24 }}>
              <AvatarSVG name={selectedAvatar} size={120} className="glow-emerald" />
            </div>
            
            <h2 style={{ margin: "0 0 8px 0", fontSize: 32, fontWeight: 800, color: "var(--accent-emerald)" }} className="text-glow-emerald">PROFILE INITIALIZED</h2>
            <p style={{ color: "var(--muted)", marginBottom: 32 }}>You are ready to enter the matching pool.</p>

            <div style={{ background: "rgba(0, 255, 179, 0.05)", border: "1px solid rgba(0, 255, 179, 0.2)", borderRadius: 16, padding: 24, marginBottom: 32, textAlign: "left" }}>
            ...
            <div style={{ display: "flex", gap: 16 }}>
              ...
              <button type="submit" className="pill-btn" style={{ flex: 1, background: "var(--accent-emerald)", color: "#000", fontWeight: 700, boxShadow: "var(--glow-emerald)" }} disabled={pending}>
