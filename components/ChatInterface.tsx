...
export function ChatInterface({
  ...
  return (
    <>
      ...
      `}</style>
      <div className="chat-container-responsive glass">
              ...
              return (
                ...
                >
                  <div style={{ position: "relative" }}>
                    <div style={{ width: 44, height: 44, borderRadius: "50%", background: "var(--surface-inset)", display: "flex", alignItems: "center", justifyContent: "center", border: "1px solid var(--stroke)", color: "var(--text-bright)", fontWeight: 700, fontSize: 18, boxShadow: isActive ? "0 0 16px rgba(180, 77, 255, 0.3)" : "none" }}>
                      {initial}
                    </div>
                    <span style={{ position: "absolute", bottom: 0, right: 0, width: 12, height: 12, borderRadius: "50%", background: "var(--accent-emerald)", border: "2px solid #080810" }} />
                  </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "rgba(8, 8, 16, 0.6)" }}>
          {activePartner ? (
            <>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                  <div style={{ width: 48, height: 48, borderRadius: "50%", background: "var(--surface-inset)", display: "flex", alignItems: "center", justifyContent: "center", border: "1px solid var(--stroke)", color: "var(--text-bright)", fontWeight: 700, fontSize: 20 }}>
                    {activePartner.partner.codename.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <h3 style={{ margin: "0 0 4px 0", fontSize: 18, fontWeight: 800, color: "var(--text-bright)" }}>
                      Chat with {activePartner.partner.codename}
                    </h3>
                    <span style={{ fontSize: 12, color: "var(--accent-emerald)", display: "flex", alignItems: "center", gap: 6, fontWeight: 600 }}>
                      <span style={{ display: "inline-block", width: 6, height: 6, borderRadius: "50%", background: "var(--accent-emerald)", boxShadow: "0 0 8px var(--accent-emerald)" }} />
                ...
                <button
                  ...
                  style={{ fontSize: 13, padding: "8px 16px", background: "var(--text-bright)", color: "#000", fontWeight: 700 }}
                >
                  Propose Partnership ⚡
                </button>
              </div>
              ...
              >
                {messages.length === 0 && contracts.length === 0 ? (
                  <div style={{ margin: "auto", textAlign: "center", color: "var(--dim)", padding: 40, background: "rgba(255,255,255,0.02)", borderRadius: 24, border: "1px solid var(--stroke-subtle)" }}>
                    <div style={{ fontSize: 32, marginBottom: 16 }}>🔒</div>
                    <p style={{ margin: "0 0 8px 0", fontSize: 16, color: "var(--text-bright)", fontWeight: 700 }}>
                  ...
                  return (
                    <div key={msg.id} style={{ alignSelf: isMe ? "flex-end" : "flex-start", maxWidth: "75%" }}>
                      <div className={isMe ? "bubble-me" : "bubble-them"} style={{ 
                        ...
                        background: isMe ? "rgba(180, 77, 255, 0.15)" : "var(--surface-inset)",
                        border: isMe ? "1px solid var(--accent-purple)" : "1px solid var(--stroke)",
                ...
                {contracts.map((ctr) => (
                  <div
                    ...
                    style={{
                      ...
                      padding: 32,
                      textAlign: "center",
                      border: ctr.status === "accepted" ? "1px solid var(--accent-emerald)" : "1px solid var(--stroke)",
                  ...
                  >
                    {ctr.status === "accepted" && (
                      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 4, background: "var(--accent-emerald)", boxShadow: "0 0 16px var(--accent-emerald)" }} />
                    )}
                    
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, marginBottom: 12 }}>
                      <span style={{ fontSize: 24 }}>🤝</span>
                      <h4 style={{ margin: 0, fontSize: 20, color: "var(--text-bright)", fontWeight: 800 }}>
                    ...
                    >
                      <div style={{ fontSize: 11, fontWeight: 700, color: "var(--text-bright)", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.05em" }}>Deliverables / Scope</div>
                      {ctr.deliverables}
                    </div>
                    ...
                    <div
                      style={{
                        fontSize: 32,
                        fontWeight: 800,
                        color: ctr.status === "accepted" ? "var(--accent-emerald)" : "var(--accent-purple)",
                    ...
                    <div>
                      {ctr.status === "pending" && String(ctr.proposed_to) === String(currentUserId) ? (
                        <div style={{ display: "flex", gap: 12, marginTop: 24 }}>
                          ...
                          <button
                            type="button"
                            className="pill-btn"
                            style={{ flex: 1, background: "var(--accent-emerald)", color: "#000", fontWeight: 700 }}
                            disabled={isPending}
                      ...
                      {ctr.status === "accepted" ? (
                        <a
                          ...
                          className="pill-btn"
                          style={{ display: "block", marginTop: 24, textDecoration: "none", background: "var(--accent-emerald)", color: "#000", fontWeight: 800, padding: 16, borderRadius: 32 }}
                        >
                          Enter Workspace 🚀
                        </a>
                      ) : null}
                      ...
                      {ctr.status === "declined" ? (
                        <p style={{ margin: "16px 0 0", fontSize: 14, color: "var(--accent-rose)", fontWeight: 700 }}>
                          Proposal Declined
                        </p>
                      ) : null}
                    </div>
              ...
              {showPropose ? (
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                    <h4 style={{ margin: 0, fontSize: 18, color: "var(--text-bright)", fontWeight: 800 }}>⚡ Micro-Contract Proposal</h4>
                    <button
                      type="button"
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                      {CONTRACT_TEMPLATES.map((t) => (
                        <button
                          ...
                          style={{ fontSize: 13, padding: "8px 16px", borderRadius: 32, background: selectedTemplate.key === t.key ? "var(--accent-purple)" : "transparent", color: selectedTemplate.key === t.key ? "#fff" : "var(--text)" }}
                        >
                          {t.label}
                        </button>
                      ))}
                  ...
                  <form action={submitContract} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                    ...
                    <button type="submit" disabled={isPending} className="pill-btn" style={{ background: "var(--text-bright)", color: "#000", fontWeight: 800, padding: 16, borderRadius: 32, fontSize: 15, marginTop: 8 }}>
                ...
                >
                  ...
                  <button
                    ...
                    className="pill-btn"
                    style={{ height: 48, padding: "0 32px", background: inputText.trim() ? "var(--text-bright)" : "var(--surface)", color: inputText.trim() ? "#000" : "var(--muted)", fontWeight: 800, borderRadius: 32, transition: "all 0.2s" }}
                  >
                    Send
                  </button>
                </div>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "var(--dim)", background: "rgba(255,255,255,0.02)" }}>
              ...
              <p style={{ fontSize: 18, color: "var(--text-bright)", fontWeight: 700, margin: "0 0 8px 0" }}>No active comm links selected</p>
